...
public abstract class Perceive {
  ...
  public static class ComponentImpl implements Perceive.Component, Perceive.Parts {
    private final Perceive.Requires bridge;
    
    private final Perceive implementation;
  ...
  private Perceive.ComponentImpl selfComponent;
   ...
   * 
   */
  protected Perceive.Provides provides() {
   ...
   * 
   */
  protected Perceive.Requires requires() {
    assert this.selfComponent != null: "This is a bug.";
    if (!this.init) {
   ...
   * 
   */
  protected Perceive.Parts parts() {
    assert this.selfComponent != null: "This is a bug.";
    if (!this.init) {
   ...
   * 
   */
  public synchronized Perceive.Component _newComponent(final Perceive.Requires b, final boolean start) {
    if (this.init) {
    	throw new RuntimeException("This instance of Perceive has already been used to create a component, use another one.");
    }
    this.init = true;
    Perceive.ComponentImpl  _comp = new Perceive.ComponentImpl(this, b, true);
    if (start) {
    	_comp.start();
    }
    return _comp;
